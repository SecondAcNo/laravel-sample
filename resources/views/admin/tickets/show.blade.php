    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto space-y-6 sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900">{{ str_replace('_', ' ', $ticket->type) }}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Closed</dt>
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.tickets.assignee.update', $ticket) }}" class="mt-4 space-y-4">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach ($supportAgents as $supportAgent)
                                <option value="{{ $supportAgent->id }}" @selected((string) old('assignee_id', $ticket->assignee_id) === (string) $supportAgent->id)>
                                    {{ $supportAgent->name }} ({{ $supportAgent->email }})
                                </option>
                            @endforeach
                        </select>
                        @error('assignee_id')
                    ...
                    <div>
                        <x-primary-button>Update Assignee</x-primary-button>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.tickets.status.update', $ticket) }}" class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
                    @csrf
                    @method('PATCH')
