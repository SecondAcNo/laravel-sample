            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('tickets.store') }}" class="space-y-6 p-6">
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <select id="type" name="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                <option value="access_request" @selected(old('type') === 'access_request')>権限申請</option>
                                <option value="incident" @selected(old('type') === 'incident')>障害報告</option>
                                <option value="general_request" @selected(old('type', 'general_request') === 'general_request')>通常依頼</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="priority" name="priority" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                <option value="normal" @selected(old('priority', 'normal') === 'normal')>通常</option>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="8" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description') }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('description')" />
                    </div>

                    <div class="flex items-center justify-end gap-3">
                        <a href="{{ route('tickets.index') }}" class="text-sm text-gray-600 hover:text-gray-900">キャンセル</a>
